...
export default function HomePageClient({ initialArticles }: { initialArticles: Article[] }) {  const [activeSection, setActiveSection] = useState("inicio")
...
// Toma los artículos que vienen de Notion (initialArticles) y les añade la clase de gradiente.
const allArticles: Article[] = initialArticles.map((article, index) => ({
  ...article,
            ...
            >
              <div className="flex items-center justify-between p-6 border-b border-white/10">
                ...
                <button
                  ...
                  className="p-2 text-white hover:text-[#DC143C] focus:text-[#DC143C] transition-colors duration-200 focus:outline-none focus:ring-2 focus:ring-[#DC143C] focus:ring-offset-2 focus:ring-offset-transparent rounded-md"
                  ...
                  {navigationItems.map((item) => (
                    <li key={item.id}>
                      <button
                        ...
                        className={`w-full text-left text-lg font-medium transition-all duration-200 hover:text-[#DC143C] focus:text-[#DC143C] focus:outline-none focus:ring-2 focus:ring-[#DC143C] focus:ring-offset-2 focus:ring-offset-transparent px-4 py-3 rounded-md hover:bg-white/5 focus:bg-white/5 ${
                          activeSection === item.id ? "text-[#DC143C] bg-white/10" : "text-[#eaeaea]"
                        }`}
                      >
                        {item.label}
                      </button>
        ...
        <div className="max-w-7xl mx-auto px-4">
            ...
            {selectedCategory !== "all" && (
              <div className="mt-6">
                <button
                  ...
                  className="inline-flex items-center px-4 py-2 bg-white/10 hover:bg-white/20 focus:bg-white/20 text-white rounded-lg transition-all duration-200 focus:outline-none focus:ring-2 focus:ring-[#DC143C] focus:ring-offset-2 focus:ring-offset-transparent"
                >
          ...
          <AnimatePresence>
            {isLoading && (
              ...
              >
                <div className="flex items-center space-x-3 text-[#DC143C]">
          ...
          {!isLoading && hasMoreArticles() && (
            <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} className="text-center mt-12">
              <button
                ...
                className="px-8 py-4 bg-[#DC143C] text-white font-semibold rounded-lg hover:bg-[#dc143c] focus:bg-[#dc143c] transition-all duration-200 border-2 border-[#DC143C] hover:border-[#dc143c] focus:border-[#dc143c] focus:outline-none focus:ring-2 focus:ring-[#DC143C] focus:ring-offset-2 focus:ring-offset-transparent"
              >
          ...
          {!isLoading && !hasMoreArticles() && displayedArticles.length > 0 && (
            <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="text-center mt-12">
              ...
              {selectedCategory !== "all" && (
                <button
                  onClick={() => handleCategoryFilter("all")}
                  className="mt-4 px-6 py-3 bg-white/10 hover:bg-white/20 focus:bg-white/20 text-white rounded-lg transition-all duration-200 focus:outline-none focus:ring-2 focus:ring-[#DC143C] focus:ring-offset-2 focus:ring-offset-transparent"
                >
            ...
            {categories.map((category) => {
              ...
              return (
                <button
                  ...
                  className={`category-card bg-[#1c1c1c] p-8 rounded-lg border border-white/10 text-center group cursor-pointer hover:bg-[#2a2a2a] focus:bg-[#2a2a2a] transition-all duration-200 focus:outline-none focus:ring-2 focus:ring-[#DC143C] focus:ring-offset-2 focus:ring-offset-transparent ${
                    selectedCategory === category.key ? "ring-2 ring-[#DC143C] bg-[#2a2a2a]" : ""
                  }`}
                >
                  <div
                  ...
                  <h3 className="text-2xl font-bold text-white mb-2 group-hover:text-[#DC143C] group-focus:text-[#DC143C] transition-colors duration-200">
                    {category.name}
                  </h3>
                  <p className="text-[#d3d3d3] mb-4">{category.description}</p>
                  <p className="text-sm text-[#eaeaea] mb-6">{category.articles} artículos</p>
                  <span className="text-[#DC143C] font-semibold group-hover:text-white group-focus:text-white transition-colors duration-200">
                    VER ARTÍCULOS →
                  </span>
                </button>
              )
            ...
            <div className="bg-[#1c1c1c] p-8 rounded-lg border border-white/10">
              ...
              <div className="space-y-6">
                ...
                <div className="text-center">
                  <button className="px-8 py-4 bg-[#DC143C] text-white font-semibold rounded-lg hover:bg-[#dc143c] focus:bg-[#dc143c] transition-all duration-200 border-2 border-[#DC143C] hover:border-[#dc143c] focus:border-[#dc143c] focus:outline-none focus:ring-2 focus:ring-[#DC143C] focus:ring-offset-2 focus:ring-offset-transparent">
                    Descargar CV
                  </button>
                </div>
              </div>
              ...
              <form className="space-y-6">
                <div>
                  ...
                  <input
                    type="text"
                    id="name"
                    className="w-full px-4 py-3 bg-[#2a2a2a] border border-gray-600 rounded-lg text-white placeholder-[#d3d3d3] focus:outline-none focus:border-[#DC143C] focus:ring-2 focus:ring-[#DC143C] transition-all duration-200"
                ...
                <div>
                  ...
                  <textarea
                    id="message"
                    rows={5}
                    className="w-full px-4 py-3 bg-[#2a2a2a] border border-gray-600 rounded-lg text-white placeholder-[#d3d3d3] focus:outline-none focus:border-[#DC143C] focus:ring-2 focus:ring-[#DC143C] resize-none transition-all duration-200"
                    placeholder="Escribe tu mensaje aquí..."
                  ></textarea>
                </div>
                <button
                  type="submit"
                  className="w-full px-6 py-3 bg-[#DC143C] text-white font-semibold rounded-lg hover:bg-[#dc143c] focus:bg-[#dc143c] transition-all duration-200 focus:outline-none focus:ring-2 focus:ring-[#DC143C] focus:ring-offset-2 focus:ring-offset-transparent"
                >
                  Enviar Mensaje
                </button>
              </form>
